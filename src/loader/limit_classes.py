...
def limit_classes(target_json, classes_taken):
    ...
    class_labels, class_counts = np.unique(list(map(
        lambda x: x['category_id'], target_json['annotations'])), 
        return_counts=True)
...
def limit_files(working_dir, directory_train, directory_targets, classes_count):
    ...
    with open(os.path.join(working_dir, 'data', directory_train, 'annotations.json'), 'r') as json_file:
    ...
    for directory_target in directory_targets:
        with open(os.path.join(working_dir, 'data', directory_target, 'annotations.json'), 'r') as json_file:
        ...
        print(len(data_json['annotations']),
            ...
            'categories.')
        with open(os.path.join(working_dir, 'data', directory_target, 'processed_annotations.json'), 'w') as outfile: 
